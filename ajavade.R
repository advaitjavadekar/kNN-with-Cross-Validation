...
# Student Name: Advait Javadekar
# Student Unity ID: ajavade
######
...
# divide training data into k folds, use one fold for validation, others for training
# repeat this k times
myknncv <- function(train, test, cl, k, numFolds)
{
   # create k folds
   ###
   folds <- createFolds(rownames(train),numFolds)
...
# pick the optimal number of nearest neighbors from the plot
###
bestk <- which.max(accuracies)+1
